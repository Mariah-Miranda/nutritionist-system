...
<div class="flex flex-col lg:flex-row gap-8">
    ...
    <div class="flex-1">
        <!-- Top Action Bar -->
        <div class="bg-white rounded-lg shadow-md p-4 mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <!-- View Tabs -->
            <div class="flex flex-wrap gap-2 justify-center sm:justify-start">
                <a href="?view=today" class="px-3 py-2 rounded-lg text-sm sm:text-base <?php echo ($view === 'today') ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'; ?> font-semibold hover:bg-blue-600 hover:text-white transition-colors duration-200 flex-grow sm:flex-grow-0 text-center">
                    Today's Schedule
                </a>
                <a href="?view=upcoming" class="px-3 py-2 rounded-lg text-sm sm:text-base <?php echo ($view === 'upcoming') ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'; ?> font-semibold hover:bg-green-600 hover:text-white transition-colors duration-200 flex-grow sm:flex-grow-0 text-center">
                    Upcoming
                </a>
                <a href="?view=weekly" class="px-3 py-2 rounded-lg text-sm sm:text-base <?php echo ($view === 'weekly') ? 'bg-purple-500 text-white' : 'bg-gray-200 text-gray-700'; ?> font-semibold hover:bg-purple-600 hover:text-white transition-colors duration-200 flex-grow sm:flex-grow-0 text-center">
                    Weekly View
                </a>
            </div>
            <!-- Action Buttons -->
            <div class="flex flex-wrap gap-2 justify-center sm:justify-end">
                <a href="<?php echo BASE_URL; ?>calendar.php" class="px-3 py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors duration-200 flex items-center justify-center space-x-2 text-sm sm:text-base flex-grow sm:flex-grow-0">
                    <i class="fas fa-calendar-alt"></i>
                    <span>Calendar View</span>
                </a>
                <a href="<?php echo BASE_URL; ?>schedule.php" class="px-3 py-2 rounded-lg bg-green-600 text-white font-semibold hover:bg-green-700 transition-colors duration-200 flex items-center justify-center space-x-2 text-sm sm:text-base flex-grow sm:flex-grow-0">
        ...
        <div id="today-schedule" class="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                <h3 class="text-lg sm:text-xl font-semibold text-gray-800">Today's Appointments</h3>
                <span class="text-sm sm:text-base text-gray-500"><?php echo date('F j, Y'); ?></span>
            </div>
            <div class="space-y-4">
                <?php if (!empty($todayAppointments)): ?>
                    <?php foreach ($todayAppointments as $appointment): ?>
                        <div class="border rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm
                            <?php
                            ...
                            <div class="flex items-center space-x-3 mb-2 sm:mb-0">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-800 text-sm sm:text-base"><?php echo htmlspecialchars($appointment['patient_name']); ?></p>
                                    <p class="text-xs sm:text-sm text-gray-600"><?php echo htmlspecialchars($appointment['reason']); ?></p>
                            ...
                            <div class="flex items-center space-x-3 w-full sm:w-auto justify-end">
                                ...
                                <div class="relative group">
                                    ...
                                    <div class="absolute right-0 mt-2 w-40 sm:w-48 bg-white rounded-md shadow-lg py-1 z-10 hidden group-hover:block">
                                        ...
                                        <a href="<?php echo BASE_URL; ?>appointments/process_appointment.php?delete_id=<?php echo $appointment['appointment_id']; ?>" onclick="showCustomConfirm('Are you sure you want to delete this appointment?', function(c) { if(c) location.href='<?php echo BASE_URL; ?>appointments/process_appointment.php?delete_id=<?php echo $appointment['appointment_id']; ?>'; });" class="block px-4 py-2 text-sm text-red-700 hover:bg-gray-100">Delete</a>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div id="upcoming-schedule" class="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Upcoming Appointments (Next 7 Days)</h3>
            <div class="space-y-4">
                <?php if (!empty($groupedUpcomingAppointments)): ?>
                    <?php foreach ($groupedUpcomingAppointments as $date => $appointmentsOnDate): ?>
                        <div class="border rounded-lg p-3 sm:p-4 shadow-sm bg-gray-50">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2">
                                <h4 class="font-semibold text-gray-700 text-sm sm:text-base"><?php echo date('F j, Y', strtotime($date)); ?></h4>
                                <span class="px-2 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 mt-2 sm:mt-0">
                            ...
                            <ul class="space-y-2">
                                <?php foreach ($appointmentsOnDate as $appt): ?>
                                    <li class="flex flex-col sm:flex-row items-start sm:items-center justify-between text-sm text-gray-600">
                                        <span>
                                        ...
                                        <?php if ($appt['status'] === 'Scheduled' && strtotime($appt['appointment_date']) <= strtotime('+2 days')): ?>
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800 mt-1 sm:mt-0">Reminder Due</span>
                                        <?php endif; ?>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
    ...
    <div class="w-full lg:w-80 space-y-6 mt-6 lg:mt-0">
        <!-- Today's Summary -->
        <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Today's Summary</h3>
        ...
        <!-- Quick Actions -->
        <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Quick Actions</h3>
            <div class="space-y-3">
                <a href="<?php echo BASE_URL; ?>schedule.php" class="flex items-center space-x-3 p-3 rounded-lg bg-green-100 text-green-700 font-medium hover:bg-green-200 transition-colors duration-200 text-sm sm:text-base">
                    <i class="fas fa-plus-circle text-lg"></i>
                    <span>New Appointment</span>
                </a>
                <a href="<?php echo BASE_URL; ?>reschedule.php" class="flex items-center space-x-3 p-3 rounded-lg bg-blue-100 text-blue-700 font-medium hover:bg-blue-200 transition-colors duration-200 text-sm sm:text-base">
                    <i class="fas fa-calendar-check text-lg"></i>
                    <span>Reschedule</span>
                </a>
                <a href="<?php echo BASE_URL; ?>send_reminders.php" class="flex items-center space-x-3 p-3 rounded-lg bg-yellow-100 text-yellow-700 font-medium hover:bg-yellow-200 transition-colors duration-200 text-sm sm:text-base">
                    <i class="fas fa-bell text-lg"></i>
                    <span>Send Reminders</span>
                </a>
                <a href="<?php echo BASE_URL; ?>export_schedules.php" class="flex items-center space-x-3 p-3 rounded-lg bg-purple-100 text-purple-700 font-medium hover:bg-purple-200 transition-colors duration-200 text-sm sm:text-base">
